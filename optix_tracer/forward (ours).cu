...
__device__ void compute_transmat_uv(
    ...
    // float dpt = sumf3(normal * (mean - ray_oc)) / sumf3(ray_dc * normal);
...
extern "C" __global__ void __raygen__ot()
{
    ...
    while (1)
    {
        ...
        if (payload.cnt != 0)
        {
            chunk_buffer[chunk_cnt++] = payload.dpt;
        }
    ...
    params.out_attr_uint32[NUM_CHANNELS_I * tidx + CHUNK_NUM_OFFSET] = chunk_cnt;
