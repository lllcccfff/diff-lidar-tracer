...
extern "C" __global__ void __raygen__ot()
{
	...
	float deepest = params.out_attr_float32[NUM_CHANNELS_F * tidx + DEEPEST_OFFSET];
	float3 ray_o_forward = params.ray_o[tidx];
    float3 ray_om = ray_o_forward + (deepest+0.1) * params.ray_d[tidx];
    ...
    while (1)
    {
        ...
        for (int i = 0; i < CHUNK_SIZE; i++)
        {
			...
			float dpt_forward = deepest + STEP_EPSILON - dpt;
			if (dpt_forward < 0)
				break;
			...
			last_depth = dpt_forward;
			dL_dalpha += (dpt_forward - acc_depths) * dL_ddpt;
			...
			if (last_gidx == first_idx_forward)
				break;
    	}
		if (last_gidx == first_idx_forward)
			break;
		if (payload.cnt < CHUNK_SIZE || dpt > (deepest+STEP_EPSILON))
			break;
