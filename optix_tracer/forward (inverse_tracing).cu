...
extern "C" __global__ void __raygen__ot()
{
    ...
    while (1)
    {
        ...
        for (int i = 0; i < CHUNK_SIZE; i++)
        {
            ...
            if (gidx == last_gidx)
            {
                // printf("dt = %f %f xyz = %f %f %f alpha %f\n", dpt - last_d, dpt, xyz.x - last_xyz.x, xyz.y - last_xyz.y, xyz.z - last_xyz.z, alpha);
                continue;
            }
            last_gidx = gidx;
            // last_d = dpt;
    ...
    params.out_attr_float32[NUM_CHANNELS_F * tidx + DEEPEST_OFFSET] = last_dpt;

    
    params.out_attr_uint32[NUM_CHANNELS_I * tidx + N_CONTRIB_OFFSET] = last_contributor;
